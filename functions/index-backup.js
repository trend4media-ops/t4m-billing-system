...
const upload = multer({
  ...
  // Removed fileFilter to reduce parser conflicts - will validate file type in handler
});
...
// Enhanced earnings endpoints using data persistence
app.get('/managers/earnings', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`📊 Fetching all managers earnings for period: ${month}`);
    const earningsData = await getAllManagersEarnings(month);
...
// Enhanced transactions routes using data persistence
...
// Upload routes (Excel processing) - ENHANCED IMPLEMENTATION
app.post('/uploads/excel', authenticateToken, (req, res) => {
  // Handle multipart upload with better error catching
  ...
  try {
    ...
    if (!req.file) {
      console.error('❌ No file in request - checking if file data exists in body');
      
      // Check if file data somehow ended up in body (FormData parsing issue)
      if (req.body.file && typeof req.body.file === 'object') {
        console.log('🔄 Found file object in body, but it should be in req.file');
        console.log('📦 Body file object:', req.body.file);
      }
    ...
    const hasValidMimeType = validMimeTypes.includes(req.file.mimetype);
    ...
    if (!periodRegex.test(period)) {
      return res.status(400).json({ 
        error: 'Period must be in YYYYMM format (e.g., "202506")',
        received: period
      });
    }

    // Parse comparison parameter
    const comparison = req.body.comparison === 'true' || req.body.comparison === true;
    console.log(`📊 Processing upload for period: ${period}, comparison: ${comparison}`);
...
// Implementation according to COMMISSION_LOGIC_DEFINITIVE.md with performance optimizations
async function processCommissionData(rows, batchId) {
  console.log(`💰 Processing commission data for ${rows.length} rows with OPTIMIZED logic`);
  ...
  // Progress tracking
  const progressRef = db.collection('uploadProgress').doc(batchId);
  await updateProgress(progressRef, 'PARSING', 'Excel file parsed, starting processing...', 0, rows.length);

  // Manager caching to avoid redundant queries
  const managerCache = new Map();
  const creatorCache = new Map();
  
  // Prepare batch operations
  const batchOperations = [];
  const BATCH_SIZE = 100; // Process in chunks of 100 rows

  // Process rows in parallel chunks for better performance
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    const chunk = rows.slice(i, i + BATCH_SIZE);
    await updateProgress(progressRef, 'PROCESSING', `Processing rows ${i + 1}-${Math.min(i + BATCH_SIZE, rows.length)}...`, i, rows.length);
    
    // Process chunk in parallel
    const chunkResults = await Promise.all(chunk.map(async (row, index) => {
...
// Enhanced bonuses routes using data persistence
...
// Enhanced upload batches API with persistence
app.get('/uploads/batches', authenticateToken, async (req, res) => {
  try {
    ...
    const batches = await getRecentUploads(limit);
...
// Get upload progress for real-time tracking
app.get('/uploads/progress/:batchId', authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
    }
    ...
    const progressDoc = await db.collection('uploadProgress').doc(batchId).get();
    ...
    res.json({
      ...progressData,
      timestamp: progressData.timestamp?.toDate?.()?.toISOString() || progressData.timestamp
    });
  } catch (error) {
...
// Admin API: Get comparison uploads - NEW ENDPOINT TO FIX 404
app.get('/uploads/comparison-uploads', authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
    }

    // Query Firestore for comparison uploads (stored in managerMonthlyNet collection)
    const snapshot = await db.collection('managerMonthlyNet')
      .orderBy('uploadedAt', 'desc')
      .get();
    ...
    snapshot.forEach(doc => {
      ...
      if (!comparisonMap.has(period)) {
        comparisonMap.set(period, {
          period: period,
          managerCount: 0,
          firstUpload: data.uploadedAt?.toDate?.() || new Date(),
          lastUpload: data.uploadedAt?.toDate?.() || new Date(),
          fileName: `comparison-${period}.xlsx`
        });
      }
      
      const periodData = comparisonMap.get(period);
      periodData.managerCount++;
      
      const uploadTime = data.uploadedAt?.toDate?.() || new Date();
      if (uploadTime < periodData.firstUpload) {
        periodData.firstUpload = uploadTime;
      }
      if (uploadTime > periodData.lastUpload) {
        periodData.lastUpload = uploadTime;
      }
    });

    // Convert map to array and format dates
    const comparisonUploads = Array.from(comparisonMap.values()).map(item => ({
      ...item,
      firstUpload: item.firstUpload.toISOString(),
      lastUpload: item.lastUpload.toISOString()
    }));

    console.log(`📊 Returning ${comparisonUploads.length} comparison upload periods`);
    res.json(comparisonUploads);

  } catch (error) {
    console.error('Error fetching comparison uploads:', error);
    res.status(500).json({ error: 'Failed to fetch comparison uploads' });
  }
});

// Progress tracking function for real-time updates
async function updateProgress(progressRef, stage, message, current, total) {
  const progressData = {
    stage: stage,
    message: message,
    current: current,
  ...
  try {
    ...
    console.log(`📊 Progress: ${stage} - ${message} (${current}/${total})`);
  } catch (error) {
    console.error('Error updating progress:', error);
  }
}

// Optimized manager and creator caching functions
async function getOrCreateManagerCached(managerCache, managerName, type, batchId) {
  ...
  const liveMgrRef = db.collection('managers').doc(managerId);
  const liveMgrDoc = await liveMgrRef.get();
  ...
  if (!liveMgrDoc.exists) {
    ...
    await liveMgrRef.set(managerData);
    console.log(`👥 Created new ${type} manager: ${managerName}`);
  } else {
    managerData = { id: managerId, ...liveMgrDoc.data() };
    await liveMgrRef.update({ handle: managerName });
  }
